...
print("Hand size before:", len(my_state.hand))

# Evolve (index 19)
s1 = search_step(search_state.searchId, [19])
# Attack (index 19 in new options)
s2 = search_step(search_state.searchId, [19])

my_s = s2.observation.current.players[s2.observation.current.yourIndex]
print("Hand size after evolve+attack:", len(my_s.hand) if my_s.hand else 0)
